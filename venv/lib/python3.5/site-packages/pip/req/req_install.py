...
class InstallRequirement(object):
    ...
    def run_egg_info(self):
        ...
        if not self.req:
            ...
            if canonicalize_name(self.req.name) != metadata_name:
                logger.warning(
                    'Running setup.py (path:%s) egg_info for package %s '
                    'produced metadata for app name %s. Fix your '
                    '#egg=%s fragments.',
    ...
    def uninstall(self, auto_confirm=False):
        ...
        if (egg_info_exists and dist.egg_info.endswith('.egg-info') and
            ...
            warnings.warn(
                "Uninstalling a distutils installed app ({0}) has been "
                "deprecated and will be removed in a future version. This is "
                "due to the fact that uninstalling a distutils app will "
                "only partially uninstall the app.".format(self.name),
...
def parse_editable(editable_req, default_vcs=None):
    ...
    if '+' not in url:
        if default_vcs:
            ...
            raise InstallationError(
                '%s should either be a path to a local app or a VCS url '
